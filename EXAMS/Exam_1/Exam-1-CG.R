...
#Add loses curves WITHOUT standard error shading
...
A_states %>% 
  ...
  geom_smooth(method = "loess", col="deeppink3", se = FALSE) + 
...
cumulative_deaths_us <- data %>%
  group_by(Last_Update) %>%
  summarize(cumulative_deaths = sum(Deaths))

cumulative_deaths_us$Last_Update<-as.Date(cumulative_deaths_us$Last_Update)

# Create the plot
cumulative_deaths_us %>% 
ggplot(mapping= aes(x = Last_Update, y = cumulative_deaths)) +
  geom_point(col="darkorchid4", alpha=0.25, size=2) +  
  labs(x = "Time", y = "Cumulative Deaths (US)") + 
  ggtitle("Cumulative Deaths (US)")+
  theme_minimal()
