...
selected_columns <- colnames(select(utah, c(-"County", -"Pop_2010")))
...
#buddhist1 = utah[utah$Buddhism.Mahayana > 0,]

utah %>% 
  pivot_longer(col = selected_columns,
               names_to =  "Religion",
               values_to = "Proportion") %>% 
  filter( Religion == "Buddhism.Mahayana", Proportion >0)  
...
utah %>% 
  ...
  group_by(Group) %>% 
  summarise(
    Mean.Religion = mean(Religious),
    Mean.Pop = mean(Pop_2010)
  )

## make data frame of each group and mean religiosity ####
#religiosity = data.frame(Pop.Group = c("group1","group2","group3","group4","group5","group6"), Mean.Religiosity = c(group1,group2,group3,group4,group5,group6),Mean.Pop = c(group1.pop,group2.pop,group3.pop,group4.pop,group5.pop,group6.pop))
...
religiosity %>% 
  ggplot(mapping = aes( x = Mean.Pop, y = Mean.Religion))+
  geom_point(color="darkorchid3")+
  labs(x = "Mean Population", y = "Mean Proportion Religion") +
  theme_minimal()+
  ggtitle("Mean Population v Mean Proportion")
...
# The religious group that correlates most strongly in a given area with the proportion of non-religious people is LDS

# 2.  What is the direction of that correlation?

# The direction of the correlation between a given area with the proportion of non-religious people for LDS is positive

# 3.  What can you say about the relationships shown here?

# For most of the religions there is not a very strong linear relationships to between given areas and the proportion of non-religious people. This is shown by mys of them graphs being flat with many data points not following the line very closely and mostly clumping up on one side. Many points are also outside the error bands. The clear one exceptions is LDS. I can say that the linear relationship between a given area and the proportion of non-religious people grouped by different regligions is not very strong except for LDS.

# 4.  Examine the axis scales. How could you modify the code above to more accurately portray values on an "equal footing?"

# I could get ride of the scales = free inside the facet wrap so that they are more directly comparable I could also get ride of the facet wrap and put everything on one graph though this might get overwhelming and hard read.
